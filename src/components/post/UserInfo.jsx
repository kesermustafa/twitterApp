...
const UserInfo = ({tweet}) => {
    ...
    return (
        <div className="user-info flex gap-3 items-center whitespace-nowrap text-gray-500">
            <p className='text-white max-sm:text-[12px]'>{tweet.user.name}</p>
            <p className='text-sm max-sm:text-[10px]'>@{userNickName}</p>
            <p className='text-sm max-sm:text-[10px]'>{date}</p>
